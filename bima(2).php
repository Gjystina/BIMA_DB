...
$sql = "CREATE TABLE IF NOT EXISTS `heilkraeuter` (
	`id_H` INT NOT NULL,
...
$sql = "CREATE TABLE IF NOT EXISTS `Rezepte` (
	`id_R` INT NOT NULL,
...
$sql = "CREATE TABLE IF NOT EXISTS `Kategorien` (
  ...
  $conn->exec($sql);
	...
	DROP TABLE IF EXISTS `heilkraeuter_has_Kategorien` ;
$sql = "CREATE TABLE IF NOT EXISTS `heilkraeuter_has_Kategorien` (
	...
	DROP TABLE IF EXISTS `AndereZutaten` ;
$sql = "CREATE TABLE IF NOT EXISTS `mydb`.`AndereZutaten` (
	`id_AZ` INT NOT NULL,
	`name` VARCHAR(45) NOT NULL,
	...
	$conn->exec($sql);
	 ...
	 DROP TABLE IF EXISTS `Rezepte_has_AndereZutaten` ;
$sql = "CREATE TABLE IF NOT EXISTS `mydb`.`Rezepte_has_AndereZutaten` (
    ...
    REFERENCES `mydb`.`Rezepte` (`id_R`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Rezepte_has_AndereZutaten_AndereZutaten1`
    FOREIGN KEY (`AndereZutaten_id_AZ`)
    REFERENCES `mydb`.`AndereZutaten` (`id_AZ`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)"
	$conn->exec($sql);
	
	DROP TABLE IF EXISTS `Rezepte_has_heilkraeuter` ;
